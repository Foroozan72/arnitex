...
class SendOTPSerializer(ReCaptchaV3Serializer): #ReCaptchaV3Serializer
    ...
    def validate(self, attrs):
        super().validate(attrs)  # This validates the recaptcha_token
    ...
    def validate(self, attrs):
        ...
        if attrs.get('email') is None and attrs.get('phone_number') is None:
            ...
            if User.objects.filter(email=attrs.get('email')):
                raise CustomValidationError(_("The email already exists."))
            elif get_user_otp(email=attrs["email"]) == None:
                raise CustomValidationError(_("Your OTP has expired."))
            ...
            if User.objects.filter(phone_number=attrs.get('phone_number')):
                raise CustomValidationError(_("The phone number already exists."))
...
class LoginVerifySerializer(ReCaptchaV3Serializer):
    email = serializers.EmailField(required=False)
    phone_number = serializers.CharField(max_length=11, required=False)
    ...
    def validate(self, attrs):
        ...
        if attrs.get('email') is None and attrs.get('phone_number') is None:
            ...
            if not User.objects.filter(email=attrs.get('email')):
                raise CustomValidationError(_("The email does not exist."))
            elif get_user_otp(email=attrs["email"]) == None:
                raise CustomValidationError(_("Your OTP has expired."))
            elif get_user_otp(email=attrs["email"]) != attrs["otp"]:
                raise CustomValidationError(_("Invalid OTP."))
            ...
            if not User.objects.filter(phone_number=attrs.get('phone_number')):
                raise CustomValidationError(_("The phone number does not exist."))
            elif get_user_otp(phone_number=attrs["phone_number"]) == None:
                raise CustomValidationError(_("Your OTP has expired."))
            elif get_user_otp(phone_number=attrs["phone_number"]) != attrs["otp"]:
                raise CustomValidationError(_("Invalid OTP."))
...
class ForgetPasswordSerializer(ReCaptchaV3Serializer):
    email = serializers.EmailField(required=False)
    phone_number = serializers.CharField(max_length=11, required=False)
...
class ChangePasswordSerializer(ReCaptchaV3Serializer):
    password = serializers.CharField(max_length=20)
    new_password = serializers.CharField(max_length=20)
...
class LogoutSerializer(ReCaptchaV3Serializer):
